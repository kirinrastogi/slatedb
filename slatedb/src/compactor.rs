...
impl CompactorEventHandler {
    ...
    /// If the compactor is embedded in the same process as a writer, pass
    /// the just-written manifest directly to the writer's memtable flusher
    /// so it can advance its local state without an object store read.
    fn notify_writer_manifest_changed(&self, dirty: DirtyObject<Manifest>) {
        if let Some(flusher) = &self.memtable_flusher {
            if let Err(err) = flusher.notify_manifest_changed(dirty) {
                debug!(
    ...
    async fn finish_compaction(
        ...
        // Pass the just-written manifest directly to the writer's memtable
        // flusher (if embedded) so it can advance without an object store
        // read, and reads in the same process observe the post-compaction
        // state immediately.
        if let Ok(dirty) = self.state_writer.manifest_dirty() {
            self.notify_writer_manifest_changed(dirty);
        }
        self.maybe_schedule_compactions().await?;
        self.maybe_start_compactions().await?;
        self.stats
